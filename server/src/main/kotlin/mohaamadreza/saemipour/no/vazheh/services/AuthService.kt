...
/**
 * Authentication Service - سرویس احراز هویت
 * ثبت نام و ورود والدین
 */
object AuthService {
    ...
    fun register(request: RegisterRequest): AuthResponse {
        return transaction {
            ...
            val existingParent = Parents.selectAll().where { Parents.email eq request.email }.singleOrNull()
            if (existingParent != null) {
                return@transaction AuthResponse(false, "این ایمیل قبلاً ثبت شده است")
            }
            
            // Create parent
            val parentId = Parents.insertAndGetId {
    ...
    fun login(request: LoginRequest): AuthResponse {
        return transaction {
            val parentRow = Parents.selectAll()
                .where { Parents.email eq request.email }
                .singleOrNull()
                ?: return@transaction AuthResponse(false, "ایمیل یا رمز عبور اشتباه است")
